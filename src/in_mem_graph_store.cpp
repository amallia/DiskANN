    ...
    : AbstractGraphStore(total_pts, reserve_graph_degree)
{
    ...
    for (size_t i = 0; i < total_pts; i++)
    {
        ...
        _graph[i].reserve(reserve_graph_degree * sizeof(uint32_t));
...
{
    std::vector<uint8_t> compressed_data(neighbours.size() * sizeof(uint32_t));
    size_t compressed_size = VarIntGB<>().encodeArray(neighbours.data(), neighbours.size(), compressed_data.data());
    compressed_data.resize(compressed_size);
